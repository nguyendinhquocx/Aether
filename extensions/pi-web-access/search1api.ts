...
function baseUrlOverride(): string | null {
	return normalizeBaseUrlOverride(loadConfig().search1apiBaseUrl);
}

function searchEndpoint(): string {
	return resolveEndpointUrl(SEARCH1API_SEARCH_URL, baseUrlOverride());
}

function crawlEndpoint(): string {
	return resolveEndpointUrl(SEARCH1API_CRAWL_URL, baseUrlOverride(), true);
}
