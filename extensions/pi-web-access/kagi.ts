...
function baseUrlOverride(): string | null {
	return normalizeBaseUrlOverride(loadConfig().kagiBaseUrl);
}

function searchEndpoint(): string {
	return resolveEndpointUrl(KAGI_SEARCH_URL, baseUrlOverride());
}

function extractEndpoint(): string {
	return resolveEndpointUrl(KAGI_EXTRACT_URL, baseUrlOverride(), true);
}
...
export async function extractWithKagi(url: string, signal?: AbortSignal, options: KagiExtractOptions = {}): Promise<ExtractedContent | null> {
	...
	let response: Response;
	try {
		response = await fetchRemoteUrl(extractEndpoint(), {
			method: "POST",
