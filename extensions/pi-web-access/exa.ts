...
function baseUrlOverride(): string | null {
	return normalizeBaseUrlOverride(loadConfig().exaBaseUrl);
}

function answerEndpoint(): string {
	return resolveEndpointUrl(EXA_ANSWER_URL, baseUrlOverride());
}

function searchEndpoint(): string {
	return resolveEndpointUrl(EXA_SEARCH_URL, baseUrlOverride(), true);
}
