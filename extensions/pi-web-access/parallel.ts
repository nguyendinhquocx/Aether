...
import { getWebSearchConfigPath, normalizeBaseUrlOverride, resolveEndpointUrl } from "./utils.ts";
...
function baseUrlOverride(): string | null {
	return normalizeBaseUrlOverride(loadConfig().parallelBaseUrl);
}

function searchEndpoint(): string {
	return resolveEndpointUrl(PARALLEL_SEARCH_URL, baseUrlOverride());
}

function extractEndpoint(): string {
	return resolveEndpointUrl(PARALLEL_EXTRACT_URL, baseUrlOverride(), true);
}
...
async function fetchAndMapExtractResult(
	...
	const data = await parallelFetch(extractEndpoint(), body, signal);
...
export async function searchWithParallel(query: string, options: ParallelSearchOptions = {}): Promise<SearchResponse> {
	const data = await parallelFetch(searchEndpoint(), buildSearchRequestBody(query, options), options.signal);
