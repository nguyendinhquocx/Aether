...
function baseUrlOverride(): string | null {
	return normalizeBaseUrlOverride(loadConfig().ollamaBaseUrl);
}

function searchEndpoint(): string {
	return resolveEndpointUrl(OLLAMA_SEARCH_URL, baseUrlOverride());
}

function fetchEndpoint(): string {
	return resolveEndpointUrl(OLLAMA_FETCH_URL, baseUrlOverride(), true);
}
...
export async function extractWithOllama(url: string, signal?: AbortSignal, options: OllamaExtractOptions = {}): Promise<ExtractedContent | null> {
	...
	let response: Response;
	try {
		response = await fetchRemoteUrl(fetchEndpoint(), {
			method: "POST",
			headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
