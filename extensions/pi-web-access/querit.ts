...
function baseUrlOverride(): string | null {
	return normalizeBaseUrlOverride(loadConfig().queritBaseUrl);
}

function searchEndpoint(): string {
	return resolveEndpointUrl(QUERIT_SEARCH_URL, baseUrlOverride());
}

function contentsEndpoint(): string {
	return resolveEndpointUrl(QUERIT_CONTENTS_URL, baseUrlOverride(), true);
}
